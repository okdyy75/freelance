...
def price_graph(make_date: str) -> None:
    ...
    """ フリーランススタート """
    df = pd.read_csv(DATA_PATH + '/freelance-start.csv')
    df = df.sort_values(by=['avg_price'], ascending=False)
    labels = df['skill'].str.replace('[ぁ-んァ-ン一-龥]+', '', regex=True)  # 日本語削除
    y_list1 = df['avg_price'].round().astype(int)
    ...
    # 棒グラフに数値追記
    for x, y in zip(labels, y_list1):
        ax1.text(x, y, y, ha='center', va='bottom', fontsize='xx-small')

    # plt.show()
    fig.savefig(DATA_PATH + '/graphs/' + make_date + '/freelance-start-price.png', bbox_inches='tight', format='png', dpi=300)
...
def count_graph(make_date: str) -> None:
    ...
    # plt.show()
    fig.savefig(DATA_PATH + '/graphs/' + make_date + '/freelance-start-count.png', bbox_inches='tight', format='png', dpi=300)
...
def mix_graph(make_date: str) -> None:
    ...
    for x, y in zip(left + width, y_list2):
        ax2.text(x, y, y, ha='center', va='bottom', fontsize='xx-small')
    ...
    # plt.show()
    fig.savefig(DATA_PATH + '/graphs/' + make_date + '/freelance-start-mix.png', bbox_inches='tight', format='png', dpi=300)
